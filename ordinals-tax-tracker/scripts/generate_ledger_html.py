#!/usr/bin/env python3
"""
HTML Ledger Viewer Generator

Generates a professional HTML file to view the CPA-style ledger.
"""
...
def generate_html_ledger(ledger_csv_path, raw_json_path, output_dir, address):
    """
    Generate an HTML file to view the ledger.
    """
    # Load ledger data
    ledger_entries = []
    ...
    net_btc_balance = total_btc_received - total_btc_sent
...
<head>
    ...
    <title>Transaction Ledger - {address[:20]}...</title>
    <style>
        * {{
            margin: 0;
            padding: 0;
    ...
    <div class="container">
        ...
        <div class="summary-cards">
            ...
            <div class="card">
                ...
                <div class="value">{total_received - total_sent}</div>
                <div style="font-size: 0.7em; color: #666; margin-top: 5px;">{net_btc_balance:.8f} BTC</div>
            </div>
            <div class="card">
...
"""
    ...
    for entry in ledger_entries:
        ...
        debit_class = 'debit' if int(entry['Debit']) > 0 else ''
        credit_class = 'credit' if int(entry['Credit']) > 0 else ''
        ...
        counterpart_short = entry['Counterpart Address'][:16] + '...' if len(entry['Counterpart Address']) > 16 else entry['Counterpart Address']
        
        # Get BTC amounts (with defaults if not present)
        debit_btc = float(entry.get('Debit BTC', 0.0))
        credit_btc = float(entry.get('Credit BTC', 0.0))
        fee_btc = float(entry.get('Fee BTC', 0.0))
        
        debit_btc_class = 'debit' if debit_btc > 0 else ''
        credit_btc_class = 'credit' if credit_btc > 0 else ''
        ...
        html_content += f"""
                    <tr class="{row_class}" data-type="{entry['Type']}" data-date="{entry['Date']}" data-txid="{entry['Transaction ID']}" data-inscription="{entry['Inscription ID']}">
                        ...
                        <td class="{debit_class}">{entry['Debit']}</td>
                        <td class="{credit_class}">{entry['Credit']}</td>
                        <td class="balance">{entry['Balance']}</td>
                        <td class="{debit_btc_class}">{debit_btc:.8f}</td>
                        <td class="{credit_btc_class}">{credit_btc:.8f}</td>
                        <td>{fee_btc:.8f}</td>
                        ...
                        <td class="inscription-id" title="{entry['Counterpart Address']}">{counterpart_short}</td>
...
"""
    ...
    html_content += """
        ...
        <div class="footer">
            <p>This ledger was generated from Ordiscan API data</p>
            <p>For accounting and tax reporting purposes</p>
        </div>
    </div>
...
"""
    ...
    # Save HTML file
    html_path = os.path.join(output_dir, f"{address}_ledger.html")
    with open(html_path, 'w', encoding='utf-8') as f:
        f.write(html_content)
    
    print(f"Saved HTML ledger viewer to: {html_path}")
    return html_path
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Generate HTML viewer for CPA-style ledger"
    )
    parser.add_argument(
        '--address',
        required=True,
    ...
    ledger_csv_path = os.path.join(args.input_dir, f"{args.address}_ledger.csv")
    raw_json_path = os.path.join(args.input_dir, f"{args.address}_raw.json")
    
    if not os.path.exists(ledger_csv_path):
        print(f"Error: Ledger CSV file not found: {ledger_csv_path}")
        print("Please run generate_ledger.py first to generate the ledger.")
        sys.exit(1)
    ...
    print(f"Generating HTML ledger viewer for address: {args.address}")
    html_path = generate_html_ledger(ledger_csv_path, raw_json_path, args.output_dir, args.address)
    print(f"\n✓ HTML ledger viewer generated!")
