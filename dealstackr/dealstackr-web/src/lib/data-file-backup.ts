...
import { calculateDealScore, parseOfferValue, POINT_VALUATIONS } from './offerScoring';
import fs from 'fs';
  ...
  try {
    ...
    for (const offer of qualityFiltered) {
      ...
      if (!existing || new Date(offer.scanned_at) > new Date(existing.scanned_at)) {
        // Parse offer value to check for points
        const parsed = parseOfferValue(offer.offer_value);
        
        // Add points metadata if detected
        let pointsData = offer.points;
        if (parsed.points && !pointsData) {
          pointsData = {
            amount: parsed.points.amount,
            program: parsed.points.program,
            valueCentsPerPoint: POINT_VALUATIONS[parsed.points.program.toLowerCase()] || POINT_VALUATIONS['default'],
            estimatedValue: parsed.points.estimatedValue
          };
        }
