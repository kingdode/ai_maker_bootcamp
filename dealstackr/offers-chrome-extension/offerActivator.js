...
(function() {
  ...
  async function activateChaseOffers() {
    sendProgress('info', 'Looking for Chase offers page...');
    
    try {
      // Wait for page to load
      await delay(2000);
      
      // Chase offers are typically in a list with "Add to card" buttons
      // Selectors may need updating as Chase changes their site
      const addButtonSelectors = [
        'button[aria-label*="Add to card"]',
        'button[data-testid*="add-offer"]',
        'button:contains("Add to card")',
        '.offer-card button.add-offer',
        '[class*="AddToCard"]',
        'button.mds-button--primary:not([disabled])',
      ];
      ...
      for (const selector of addButtonSelectors) {
        try {
          ...
          if (buttons.length > 0) {
            addButtons = Array.from(buttons).filter(btn => {
              const text = btn.textContent?.toLowerCase() || '';
              const ariaLabel = btn.getAttribute('aria-label')?.toLowerCase() || '';
              return text.includes('add') || ariaLabel.includes('add');
            });
      ...
      if (addButtons.length === 0) {
        ...
        addButtons = Array.from(allButtons).filter(btn => {
          const text = btn.textContent?.toLowerCase() || '';
          return (text.includes('add to card') || text.includes('add offer')) && 
                 !btn.disabled &&
      ...
      if (addButtons.length === 0) {
        sendProgress('warning', 'No "Add to Card" buttons found. Make sure you are logged in and on the Chase offers page.');
        sendProgress('info', 'Try navigating to: chase.com/digital/offers');
        return;
      }
      ...
      for (let i = 0; i < addButtons.length; i++) {
        ...
        try {
          ...
          const card = button.closest('.offer-card, [class*="offer"], [class*="Offer"]');
          const merchantEl = card?.querySelector('[class*="merchant"], [class*="name"], h3, h4');
  ...
  // Also check URL params for auto-start
  const urlParams = new URLSearchParams(window.location.search);
  if (urlParams.get('dealstackr_activate') === 'true') {
    console.log('[DealStackr Activator] Auto-starting from URL parameter');
